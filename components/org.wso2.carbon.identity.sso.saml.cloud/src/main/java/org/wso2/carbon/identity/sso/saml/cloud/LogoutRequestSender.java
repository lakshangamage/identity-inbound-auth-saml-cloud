...
public class LogoutRequestSender {
     ...
     */
    public void sendLogoutRequests(SingleLogoutRequest[] singleLogoutRequestDTOs) {
        if (singleLogoutRequestDTOs == null) {
            return;
        }
        // For each logoutReq, create a new task and submit it to the thread pool.
        for (SingleLogoutRequest reqDTO : singleLogoutRequestDTOs) {
    ...
    private class LogoutReqSenderTask implements Runnable {
        ...
        private SingleLogoutRequest logoutReq;

        public LogoutReqSenderTask(SingleLogoutRequest logoutReq) {
        ...
        public void run() {
            ...
            decodedSAMLRequest = logoutReq.getLogoutResponse();
            ...
            if (isSAMLSOAPBindingEnabled) {
                ...
                // set the logout request
                logoutReqParams.add(new BasicNameValuePair(SAMLSSOConstants.SAML_REQUEST_PARAM_KEY, SAMLSSOUtil.encode(logoutReq.getLogoutResponse())));
            }
            ...
            try {
                ...
                HttpPost httpPost = new HttpPost(logoutReq.getAssertionConsumerURL());
                httpPost.setEntity(entity);
                httpPost.addHeader(SAMLSSOConstants.COOKIE_PARAM_KEY, SAMLSSOConstants.SESSION_ID_PARAM_KEY + logoutReq.getRpSessionId());
                if (isSAMLSOAPBindingEnabled) {
                ...
                for (int currentRetryCount = 0; currentRetryCount < SAMLSSOUtil.getSingleLogoutRetryCount(); currentRetryCount++) {
                    ...
                    if (response != null && (SAMLSSOUtil.isHttpSuccessStatusCode(statusCode) || SAMLSSOUtil
                        ...
                        if (log.isDebugEnabled()) {
                            log.debug("single logout request is sent to : " + logoutReq.getAssertionConsumerURL() +
                        ...
                        if (statusCode != 0) {
                            log.warn("Failed single logout response from " +
                                    logoutReq.getAssertionConsumerURL() + " with status code " +
                        ...
                        try {
                            ...
                            log.info("Sending single log out request again with retry count " +
                                    ...
                                    logoutReq.getAssertionConsumerURL());
                ...
                log.error("Error sending logout requests to : " + logoutReq.getAssertionConsumerURL(), e);
            }
        }
    }
